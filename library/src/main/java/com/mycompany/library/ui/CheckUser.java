...
public class CheckUser extends javax.swing.JFrame {
     ...
     */
    public CheckUser(String adminId) {
        ...
        jButton1.addActionListener((ActionEvent e) -> {
            ...
            if (!id.isEmpty()) {
                if (isValidUserId(id)) { // 檢查使用者ID是否有效
                    if(checkFine(id)>0){
                      JOptionPane.showMessageDialog(CheckUser.this, "借閱書籍逾期未還，請先繳納罰金"+checkFine(id)+"元，並歸還書籍");
                      openFineFrame(id,adminId);
                    }
                    else{
                        JOptionPane.showMessageDialog(CheckUser.this, "信用良好，可借用書籍");
                        openBorrowFrame(id,adminId);
                    }
                } else {
                    jTextField1.setText("");
                    JOptionPane.showMessageDialog(CheckUser.this, "ID輸入錯誤，請重試！", "錯誤", JOptionPane.ERROR_MESSAGE);
                }
            } else {
                JOptionPane.showMessageDialog(CheckUser.this, "請輸入ID", "錯誤", JOptionPane.ERROR_MESSAGE);
            }
        });
        }
    ...
    private void openBorrowFrame(String userId,String adminId) {
        SwingUtilities.invokeLater(() -> {
            new AdminBorrow(userId,adminId).setVisible(true);
        });
        this.dispose(); // 关闭登录窗口
    }
